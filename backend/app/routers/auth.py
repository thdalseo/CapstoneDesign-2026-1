...
def send_code(req: SendCodeRequest, db: Session = Depends(get_db)):
    """강원대 이메일로 인증 코드 발송. DB에 코드와 만료 시각 저장."""
    ...
    record = (
        ...
        .filter(EmailVerification.email == req.email)
        .first()
    )
    if record:
        record.code = code
...
def register(req: RegisterRequest, db: Session = Depends(get_db)):  # noqa: C901
    """코드 검증 + 회원 생성을 한 번에 처리."""
    # 1. 인증 레코드 확인
    record = (
        db.query(EmailVerification)
        .filter(EmailVerification.email == req.email)
...
def login(req: LoginRequest, db: Session = Depends(get_db)):
    ...
    # 유저 없거나 비밀번호 불일치 → 동일한 메시지로 응답 (보안)
